...
import { parseOptionalInt, progressFieldsForType } from "@/lib/progress";
import type {
  LeisureItem,
  LeisureProgress,
  LeisureStatus,
...
export function ItemForm({ mode, initialItem, onSave, onClose }: ItemFormProps) {
  const [type, setType] = useState<LeisureType>(initialItem?.type ?? "book");
  ...
  const endpoint = searchEndpoint(type);
  const canSearch = endpoint !== null;
  const progressFields = progressFieldsForType(type);

  const search = useCallback(
    async (q: string) => {
      if (!endpoint || q.length < 2) {
        setResults([]);
        return;
      }
  ...
  const pickResult = (result: SearchResult) => {
    ...
    if (result.sourceUrl && !watchUrl.trim()) setWatchUrl(result.sourceUrl);
    setResults([]);
  };
        ...
        <div className="overflow-y-auto px-5 py-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={canSearch ? "Start typing to auto-fill…" : "e.g. کتاب یوسف آباد"}
              className="w-full rounded-xl border border-border bg-background py-2.5 pl-10 pr-10 text-sm outline-none ring-accent/30 focus:ring-2"
            />
            {loading && (
